...
| Status display | `session_start` + `turn_end` | Shows milestone, current phase (name + status), plan, active workstream, and progress in pi's footer |
...
| Workflow guard | `gsd-hooks` extension — warns on edits inconsistent with `STATE.md next_action` |
...
| **Read guard** | `gsd-read-guard.js` | Prevents Edit/Write on unread files — provided by pi-lens |
